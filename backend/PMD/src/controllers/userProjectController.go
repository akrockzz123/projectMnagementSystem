...
func GetUsersByProjectID(c *gin.Context){
    const functionName = "controllers.GetUsersByProjectID"
    idn:= c.Param("project_id")
    id,err := strconv.Atoi(idn)
    if err != nil{
        c.JSON(http.StatusNotFound,"Invalid Project ID")
        return
    }
    u,err := model.GetUsersByProjectID(id)
    if err != nil{
